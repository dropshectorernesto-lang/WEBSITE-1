(() => {
  ...
  const replaceBrand = (doc) => {
    ...
    const cfg=doc.defaultView.SITE_CONFIG;
    if(cfg?.business){ cfg.business.name=c.name; cfg.business.title=`${c.name} — Dog Grooming`; cfg.business.description=`${c.name} dog grooming in Barcelona.`; if(c.social) cfg.business.instagramUrl=c.social; }
  ...
  const customize = (doc) => {
    ...
    const lang=langOf(doc); const copy=c.copy[lang]||c.copy.en; replaceBrand(doc);
    const cards=[...doc.querySelectorAll('.contact-info-grid .value-card')];
    if(cards.length>=4){
      const phone=cards[0].querySelector('a,p'); if(phone){if(phone.tagName==='A')phone.href=c.phoneHref;phone.textContent=c.phoneDisplay;}
    ...
    if(mapLink){ mapLink.href=c.mapsUrl; mapLink.setAttribute('aria-label',`Open ${c.name} on Google Maps`); const map=mapLink.querySelector('iframe'); if(map){map.src=c.mapEmbed;map.title=`${c.name} location on Google Maps`;} }
